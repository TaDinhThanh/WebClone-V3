...
use App\Http\Controllers\HwpCampaignController;
...
class ToolCloneController
{
        ...
        try {
            ...
            if (empty($url_check->ky_hieu) || $url_check->ky_hieu == 'w') {
                ...
                for ($i = 0; $i < count($list_url); $i++) {
                    ...
                    DB::table("hwp_urls")->where('id', '=', $list_url[$i]->id)->update(['check' => true]);
                    // lấy ra key tương ứng với url đó
                    $key = DB::table("hwp_keys")->where('id', '=', $list_url[$i]->id_key)->get()->toArray();
                ...
                DB::table("hwp_urls")->where('id', '=', $body->id)->update(['check' => true]);
            }
        } catch (\Exception $e) {
            return $e;
        }
    ...
    public function saveKey(Request $request)
    {
        ...
        for ($j = 0; $j < count($body); $j++) {
            $key = DB::table('hwp_keys')->where('ten', '=', $body[$j]->ten)->get()->toArray();
            if (count($key) == 0) {
                // lưu url key cha
                $url_key = $body[$j]->tien_to . " " . $body[$j]->ten . " " . $body[$j]->hau_to . "-vi-cb";
                DB::table('hwp_keys')->insert([
    ...
    public function saveKeyByIdCam(Request $request, $id_cam)
    {
        $body = json_decode($request->getContent());
        for ($j = 0; $j < count($body); $j++) {
            $key = DB::table('hwp_keys')->where('ten', '=', $body[$j]->ten)->where('hwp_keys.id_cam', '=', $id_cam)->get()->toArray();
            if (count($key) == 0) {
                // lưu url key cha
                $url_key = $body[$j]->tien_to . " " . $body[$j]->ten . " " . $body[$j]->hau_to . "-vi-cb";
                DB::table('hwp_keys')->insert([
    ...
    {
        DB::table('hwp_keys')->where('id', '=', $id_key)->update(['check' => true]);
    ...
    public function saveBlackList(Request $request)
    {
        $body = json_decode($request->getContent());
        for ($j = 0; $j < count($body); $j++) {
            $key = DB::table('hwp_black_lists')->where('domain', '=', $body[$j]->domain)->get()->toArray();
            if (count($key) == 0) {
                DB::table('hwp_black_lists')->insert([
    ...
    public function saveBlackListByIdCam(Request $request, $id_cam)
    {
        ...
        for ($j = 0; $j < count($body); $j++) {
            ...
            $key = DB::table('hwp_black_lists')->where('domain', '=', $body[$j]->domain)->where('hwp_black_lists.id_cam', '=', $id_cam)->get()->toArray();
    ...
    {
        $key = DB::table('hwp_keys')->where('id', '=', $id_key)->first();
        DB::table('hwp_keys')->where('id', '=', $id_key)->update(['check' => false]);
        return array(["code" => 200]);
    }

    public function resetUrl(Request $request, $id_key)
    {
        DB::table('hwp_urls')->where('hwp_urls.id_key', '=', $id_key)->update(['check' => false]);
        return array(["code" => 200]);
    }
    ...
    {
        $list_url = DB::table('hwp_urls')->where("id_key", '=', $id_key)->get()->toArray();
        return json_encode($list_url);
    }

    public function getUrlById($id_url)
    {
        $list_url = DB::table('hwp_urls')->where("id", '=', $id_url)->get()->toArray();
        return json_encode($list_url);
    }
    ...
    public function getUrlByIdKey2($id_key)
    {
        ...
        $list_url = DB::table('hwp_urls')->where("hwp_urls.id_key", '=', $id_key)->get()->toArray();
        return json_encode($list_url);
    }
    ...
    {
        $list_key = DB::table('hwp_keys')->get()->toArray();
    ...
    {
        $list_black = DB::table('hwp_black_lists')->get()->toArray();
    ...
    {
        DB::table('hwp_black_lists')->where('id', '=', $id)->delete();
        return array(["code" => 200, 'message' => 'Success']);
    }

    public function xoaAllBlackKey($id_cam)
    {
        DB::table('hwp_black_lists')->where('hwp_black_lists.id_cam', '=', $id_cam)->delete();
        return array(["code" => 200, 'message' => 'Success']);
    }

    public function xoaKey($id)
    {
        DB::table('hwp_keys')->where('id', '=', $id)->delete();
        DB::table('hwp_urls')->where("hwp_urls.id_key", '=', $id)->delete();
    ...
    {
        $list_key = DB::table('hwp_keys')->select("id")->where("id_cam", '=', $id_cam)->get()->toArray();
        if (!empty($list_key)) {
            foreach ($list_key as $key) {
                DB::table('hwp_urls')->where('hwp_urls.id_key', '=', $key->id)->delete();
                DB::table('hwp_posts')->where('hwp_posts.id_key', '=', $key->id)->delete();
            }
        }
        DB::table('hwp_keys')->where('id_cam', '=', $id_cam)->delete();
    ...
    {
        $list = DB::table('hwp_keys')->where('ten', 'like', '%' . $name . '%')->get()->toArray();
    ...
    {
        DB::table('hwp_urls')->where('id', '=', $id)->delete();
    ...
    {
        DB::table('hwp_urls')->where('id_key', '=', $id_key)->delete();
    ...
    {
        $key = DB::table('hwp_keys')->where("hwp_keys.id_cam", '=', $id)->get()->toArray();
        return json_encode($key);
    }

    public function updateCountRequest($count_y, $count_g, $id_key)
    {
        DB::table('hwp_keys')->where('id', '=', $id_key)->update(['count_request_y' => $count_y]);
        DB::table('hwp_keys')->where('id', '=', $id_key)->update(['count_request_g' => $count_g]);
        return array(["code" => 200]);
    }

    public function getTotalRequest($id_cam)
    {
        // DB::table('hwp_keys')->select("id_cam", SUM("count_request_y", co))
        $result = DB::table('hwp_keys')->where('hwp_keys.id_cam', '=', $id_cam)
            ->select([DB::raw("SUM(count_request_y) as total_y"), DB::raw("SUM(count_request_g) as total_g")])
            ->groupBy('hwp_keys.id_cam')
            ->get();
    ...
    // {
    //     $stack = array();
    //     $key = DB::table('hwp_keys')->select("hwp_keys.id")
    //     ->where("hwp_keys.id_cam", '=', $id_cam)->get()->toArray();
    ...
    {
        $video = DB::table('hwp_keys')->select("hwp_keys.id")
            ->where("hwp_keys.id_cam", '=', $id_cam)->leftJoin('hwp_urls', 'hwp_keys.id', '=', 'hwp_urls.id_key')->where("hwp_urls.ky_hieu", '=', 'y')->get()->toArray();
        return $video;
    }
    ...
    {
        $items = DB::table('hwp_keys')
            ->where("hwp_keys.id_cam", '=', $id)
            ->get()
            ->toArray();
        $list_id = array();
        foreach ($items as $i) {
            $url = DB::table('hwp_urls')
                ->select('ky_hieu')
        ...
        // $key_in_url = DB::table("hwp_urls")->select("hwp_urls.id_key")->get()->toArray();
        //        $key_in_url = DB::table('hwp_keys')
        //            ->select("hwp_keys.id", 'hwp_keys.tien_to', 'hwp_keys.ten', 'hwp_keys.hau_to', 'hwp_keys.url_key_cha',
        //                'hwp_keys.check', 'hwp_keys.id_cam', 'hwp_keys.key_con_1', 'hwp_keys.key_con_2', 'hwp_keys.key_con_3',
        //                'hwp_keys.key_con_4', 'hwp_keys.url_key_con_1', 'hwp_keys.url_key_con_2', 'hwp_keys.url_key_con_3',
        //                'hwp_keys.url_key_con_4', 'hwp_keys.top_view_1', 'hwp_keys.url_top_view_1', 'hwp_keys.top_view_2',
        //                'hwp_keys.url_top_view_2', 'hwp_keys.top_view_3', 'hwp_keys.url_top_view_3', 'hwp_keys.top_view_4',
        //                'hwp_keys.url_top_view_4', 'hwp_keys.top_view_5', 'hwp_keys.url_top_view_5', 'hwp_keys.ky_hieu', 'hwp_keys.id_list_vd')
        //                ->where("hwp_keys.id_cam", '=', $id)
        //                ->whereNotIn('hwp_keys.id',$list_id)
        //            ->get()->toArray();
    ...
    {
        $key = DB::table("hwp_keys")->select("id")->where('hwp_keys.id_cam', '=', $id_cam)->get()->toArray();
        return json_encode($key);
    }
    ...
    //     $list_url = DB::table('hwp_urls')->leftJoin('hwp_posts', 'hwp_urls.id', '=', 'hwp_posts.id_url')->where("hwp_urls.id_key", '=', $id_key)->limit(50)->get()->toArray();
    //     return json_encode($list_url);
    // }

    public function getBlackListByIdCam($id)
    {
        $black_list = DB::table("hwp_black_lists")->where("id_cam", '=', $id)->get()->toArray();
    ...
    public function deleteCam($id_cam)
    {
        ...
        DB::table('hwp_urls')
            ->join('hwp_keys', 'hwp_keys.id', '=', 'hwp_urls.id_key')
            ->join('hwp_campaigns', 'hwp_campaigns.id', '=', 'hwp_keys.id_cam')
            ->where('hwp_campaigns.id', '=', $id_cam)
            ->delete();
        ...
        if (!empty($list_key)) {
            foreach ($list_key as $key) {
                DB::table('hwp_urls')->where('hwp_urls.id_key', '=', $key->id)->delete();
                DB::table('hwp_posts')->where('hwp_posts.id_key', '=', $key->id)->delete();
                DB::table('hwp_videos')->where('hwp_videos.id_key', '=', $key->id)->delete();
            }
        }

        DB::table('hwp_keys')->where('id_cam', '=', $id_cam)->delete();
        DB::table('hwp_campaigns')->where('id', '=', $id_cam)->delete();
    ...
    {
        DB::table('hwp_campaigns')->where('id', '=', $id_cam)->update(['check' => true]);
        return array(["code" => 200]);
    }

    public function resetStatusCam($id_cam)
    {
        DB::table('hwp_campaigns')->where('id', '=', $id_cam)->update(['check' => false]);
        return array(["code" => 200]);
    }
        ...
        if ($to - $from >= 0) {
            ...
            foreach ($list_key as $th) {
                if ($th->check == 1) {
                    ...
                    /*  
                    // TODO:  Thực hiện xóa posts sau khi xuất file json và text 
                    /* 
                    DB::table('hwp_posts')
                        ->where('id_key', '=', $th->id)->delete();
                        */ 
                    DB::table('hwp_keys')->where('id', '=', $th->id)->update(['check' => false]);
                }
            }
        }
    ...
    public function createTH_rd($from, $to, $id_cam)
    {
        if ($to - $from >= 0) {
            $list_key = DB::table('hwp_keys')
                ->select('hwp_keys.id', 'tien_to', 'ten', 'hau_to', 'hwp_keys.check', 'url_key_cha', 'language')
                ->join('hwp_campaigns', 'hwp_campaigns.id', '=', 'hwp_keys.id_cam')
                ->skip($from - 1)
                ->take($to - $from + 1)
                ->where("id_cam", '=', $id_cam)
        ...
        foreach ($list_key as $key) {
            ...
            if (empty($check) && $key->check == 1) {
                ...
                //                DB::table('hwp_yoast_indexables')->where('breadcrumb_title', '=', $title)->delete();
...
<div class="non_toc" style="background: #eee; border: 1px solid #ccc; padding: 5px 10px;">';
                    ...
                    if (!empty($post_detail)) {
                        ...
                        foreach ($post_detail as $post_id) {
                            ...
                            //                        DB::table('hwp_yoast_indexables')->where('object_id', '=', $post_id->ID)->update([
                            //                            'description'=>''
                            //                        ]);;
                        }
                    }
    ...
    public function createTH_wiki($from, $to, $id_cam)
    {
        ...
        foreach ($list_key as $key) {
            if ($key->check == 1) {
                ...
                //                DB::table('hwp_posts')->where('post_title', '=', $title)->delete();
                //                DB::table('hwp_yoast_indexables')->where('breadcrumb_title', '=', $title)->delete();
                $video = DB::table('hwp_videos')->where('id_key', '=', $key->id)->first();
                if (!empty($video)) {
                ...
                if (!empty($post_detail)) {
                    if ($key->language == 'Vietnamese') {
                        ...
                        <div class="ifream">
                            <iframe loading="lazy" class="responsive-iframe" width="100%" height=""
                                    src="' . $video->link . '">
                            </iframe>
                        </div>
...
<div class="non_toc" style="border: 1px solid #ccc; padding: 5px 10px;">';
    ...
    //        $body = json_decode($request->getContent());
    //        for ($j = 0; $j < count($body); $j++) {
    //            $video = DB::table('hwp_videos')->where('id_key', '=', $body[$j]->id_key)->get()->toArray();
    //            if (count($video) == 0) {
    //                DB::table('hwp_videos')->insert([
    //                    "link" => $body[$j]->link,
    ...
    {
        $video = DB::table('hwp_videos')->where('hwp_videos.id_key', '=', $id_key)->get()->toArray();
                        ...
                        for ($i = 0; $i < count($post_detail); $i++) {
                            if ($post_detail[$i]->ky_hieu != 'y' && $post_detail[$i]->ky_hieu != 'i') {
                                $content .= '<img src = "' . $post_detail[$i]->url_image . '"
                                     alt = "' . $post_detail[$i]->url_title . '" style = "width: 760px; height: 100%;">';
                                break;
                            }
                        }
                        for ($i = 0; $i < count($post_detail); $i++) {
    ...
    {
        $key = DB::table("hwp_keys")->select("hwp_keys.id", "hwp_keys.ky_hieu", "hwp_keys.id_list_vd")->where("id", '=', $id)->get()->toArray();
        return json_encode($key);
    }
    ...
    {
        try {
            $key = DB::table("hwp_key_googles")->where("hwp_key_googles.key_api", '=', $key_api)->where("hwp_key_googles.type", '=', 'google')->get();
            $id_current = $key[0]->id;
            $key_current = $key[0]->key_api;
            $count_current = $key[0]->count;
            if ($count_current >= 0 && $count_current < 100) {
                DB::table("hwp_key_googles")->where("hwp_key_googles.id", '=', $id_current)->update(["hwp_key_googles.count" => $count_current + 1]);
            } else {
                DB::table("hwp_key_googles")->where("hwp_key_googles.id", '=', $id_current)->update(["hwp_key_googles.count" => 100]);
            }
            return array(
                'success' => true
            );
    ...
    {
        try {
            $key = DB::table("hwp_key_googles")->where("hwp_key_googles.key_api", '=', $key_api)->where("hwp_key_googles.type", '=', 'youtube')->get();
            $id_current = $key[0]->id;
            $key_current = $key[0]->key_api;
            $count_current = $key[0]->count;
            if ($count_current >= 0 && $count_current < 500) {
                ...
                DB::table("hwp_key_googles")->where("hwp_key_googles.id", '=', $id_current)->update(["hwp_key_googles.count" => 500]);
            }
            return array(
                'success' => true
            );
    ...
    {
        try {
            DB::table("hwp_key_googles")->where("hwp_key_googles.type", '=', 'google')->where("hwp_key_googles.key_api", '=', $key)->update(["hwp_key_googles.count" => 100]);
            return array(
                'success' => true
            );
        } catch (\Exception $e) {
    ...
    {
        try {
            DB::table("hwp_key_googles")->where("hwp_key_googles.type", '=', 'youtube')->where("hwp_key_googles.key_api", '=', $key)->update(["hwp_key_googles.count" => 500]);
            return array(
                'success' => true
            );
        } catch (\Exception $e) {
    ...
    {
        try {
            $key = DB::table("hwp_key_googles")->where("hwp_key_googles.type", '=', 'google')->where("count", '<', '100')->limit(1)->get()->toArray();
            return json_encode($key);
        } catch (\Exception $e) {
    ...
    {
        try {
            $key = DB::table("hwp_key_googles")->where("hwp_key_googles.type", '=', 'youtube')->where("count", '<', '500')->limit(1)->get()->toArray();
            return json_encode($key);
        } catch (\Exception $e) {
    ...
    {
        try {
            $key = DB::table("hwp_key_googles")->where("hwp_key_googles.type", '=', 'google')->get()->toArray();
            return json_encode($key);
        } catch (\Exception $e) {
            return array('code' => 500, message => 'k get được key gg');
        }
    ...
    {
        try {
            $key = DB::table("hwp_key_googles")->where("hwp_key_googles.type", '=', 'youtube')->get()->toArray();
            return json_encode($key);
        } catch (\Exception $e) {
            return array('code' => 500, message => 'k get được key yt');
        }
    ...
    {
        DB::table("hwp_key_googles")->where("hwp_key_googles.type", '=', 'google')->update(["hwp_key_googles.count" => 0]);
    }

    public function resetAllKeyYoutube()
    {
        DB::table("hwp_key_googles")->where("hwp_key_googles.type", '=', 'youtube')->update(["hwp_key_googles.count" => 0]);
    }
    ...
    public function saveKeyGoogle(Request $request)
    {
        $body = json_decode($request->getContent());
        for ($j = 0; $j < count($body); $j++) {
            $key = DB::table("hwp_key_googles")->where("hwp_key_googles.key_api", '=', $body[$j]->key_api)->get()->toArray();
            if (count($key) == 0) {
                DB::table('hwp_key_googles')->insert([
    ...
    {
        DB::table("hwp_key_googles")->where("hwp_key_googles.id", '=', $id)->delete();
        return array(
            'success' => true
        );
    }
    ...
    {
        DB::table("hwp_key_googles")->where('type', "google")->delete();
        return array(
            'success' => true
        );
    }
    ...
    {
        DB::table("hwp_key_googles")->where('type', "youtube")->delete();
        return array(
            'success' => true
        );
    }
    ...
    {
        $ky_hieu = DB::table('hwp_keys')->where('id', '=', $id_key)->first();
        ...
        for ($i = 0; $i < sizeof($list_kh); $i++) {
            if (str_contains($list_kh[$i], 'w')) {
                ...
                //                $ar[$i+1]= $num;
                DB::table('hwp_urls')->where('id_key', '=', $id_key)
